...
const VanHoaNewsReducer = (state, action) => {
    switch (action.type) {
        case "SET_LOADING":
...
const VanHoa = () => {
    const [state, dispatch] = React.useReducer(VanHoaNewsReducer, initialState);
    ...
    async function fetchData() {
        try {
            ...
            dispatch({ type: "SET_DATA_SECTION", payload: dataSection.slice(0, 12) });
